            ...
            public void run() {
                ...
                try {
                    TeleinfoInputStream teleinfoInputStream = new TeleinfoInputStream(serialPort.getInputStream(),
                            false, waitNextHeaderFrameTimeoutInMs, readingFrameTimeoutInMs) {
                        @Override
                        public void close() {
                            // don't close under input stream
                        }
                    };
                ...
                } catch (IOException | InvalidFrameException | TimeoutException e) {
                    LOGGER.error("An error occurred during teleinfo frame reading", e);
                }
            }
        }, 10, refreshInterval);
